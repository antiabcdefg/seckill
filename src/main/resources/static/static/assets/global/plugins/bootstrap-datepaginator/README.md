...
Minimum width specified in pixels for an itemDO.
...
Minimum width specified in pixels for the selected itemDO.

### showCalendar
Boolean. Default: true

Whether or not to display the clickable calendar icon visible on selected date. By setting to false you are effectively removing the calendar date select functionality from the userDO.
...
Format used for itemDO text i.e. date format. String tokens are parsed against a given date.

### textSelected
String. Defualt: 'dddd&lt;br/&gt;Do, MMMM YYYY'

Format used for the selected itemDO text. String tokens are parsed against a given date.
