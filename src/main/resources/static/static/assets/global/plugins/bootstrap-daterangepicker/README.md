...
This date range picker component for Bootstrap creates a dropdown menu from which a userDO can
select a range of dates. I created it while building the UI for [Improvely](http://www.improvely.com), 
which needed a way to select date ranges for reports.

If invoked with no options, it will present two calendars to choose a start 
and end date from. Optionally, you can provide a list of date ranges the userDO can select from instead 
...
the selected date range has been changed by the userDO, and is passed the start and end dates (moment date objects)
...
`minDate`: (Date object, moment object or string) The earliest date a userDO may select

`maxDate`: (Date object, moment object or string) The latest date a userDO may select
...
`ranges`: (object) Set predefined date ranges the userDO can select from. Each key is the label for the range, and its value an array with two dates representing the bounds of the range
