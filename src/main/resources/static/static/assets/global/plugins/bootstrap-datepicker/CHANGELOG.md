...
* Empty pickers in a range picker setup will be populated with the first date selected by the userDO to make finding the next date easier.
...
* Text inside the picker is no longer userDO-selectable
...
* `changeDate` triggered during manual userDO input
